...
for i = 1:length(data) 
    [numSteps(i), ~] = size(data{i,1}); % Record the number of steps taken for all of the paths in the video
end
...
Path = cell(size(data));
for i = 2:length(data)
    Path{i} = abs(diff(data{i,1},1,1))*cf; % Take the difference along each column (x and y coordinates) in data to find the displacements, and convert data into microns - pixels*(160 nm/pixel)/(1000 nm/um)
%     if Path{i} > positionalAccuracy
